...
float calculation(char *in, float initial, int review)
{
    ...
    {
        if (in[k] == 43 || in[k] == 45 || in[k] == 42 || in[k] == 47 || in[k] == 94)
        {
            operator[x] = in[k];
            x++;
        }
...
bool isvalid(char *in, int review)
{
    for (int i = 0; in[i] != '\0'; i++)
    {
        ...
        if (!(isdigit(in[i]) || in[i] == 43 || in[i] == 45 || in[i] == 42 || in[i] == 47 || isspace(in[i]) || in[i] == 94))
            return false;
    ...
    for (int j = 0; in[j] != '\0'; j++)
    {
        if (in[j] == 43 || in[j] == 45 || in[j] == 42 || in[j] == 47 || in[j] == 94)
        {
    ...
    {
        if (isdigit(in[0]))
            return false; // Invalid: input starts with a digit instead of an operator
